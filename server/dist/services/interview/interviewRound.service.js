...
exports.InterviewRoundService = void 0;
const linkGenerator_1 = require("../../shared/utils/linkGenerator");
const logger_1 = require("../../shared/utils/logger");
const mongoose_1 = __importDefault(require("mongoose"));
class InterviewRoundService {
    constructor(_repository, _interviewRepository, _notificationService) {
        this._repository = _repository;
        this._interviewRepository = _interviewRepository;
        this._notificationService = _notificationService;
    }
    // Create interview round
    async createRound(applicationId, roundData) {
        const roundId = new mongoose_1.default.Types.ObjectId();
        const { link, token } = (0, linkGenerator_1.generateMeetingLink)(roundId.toString());
        const roundToCreate = {
            _id: roundId,
            applicationId,
        ...
        const created = await this._repository.create(roundToCreate);
        logger_1.logger.info("Interview round created", {
            roundId: created.id,
            applicationId,
            roundNumber: roundData.roundNumber,
        });
        const roundWithDetails = await this._repository.findByIdWithDetails(created.id);
        if (!roundWithDetails) {
            throw new Error("Failed to fetch created round");
        }
        await this._syncParentInterview(applicationId);
        try {
            await this._notificationService.notifyCandidateInterviewScheduled(roundWithDetails.candidateId, roundWithDetails.job.title, roundWithDetails.scheduledDate
                ? roundWithDetails.scheduledDate.toISOString()
                : new Date().toISOString(), roundWithDetails.id, roundWithDetails.employer.companyName);
            ...
            logger_1.logger.error("Failed to notify candidate about new interview", {
                notifyError,
    ...
    async getRoundsForApplication(applicationId, query) {
        return await this._repository.findByApplicationId(applicationId, query);
    }
    //Fetching candidate round
    async getRoundsForCandidate(candidateId, query = {}) {
    ...
    async getRoundsForEmployer(employerId, query = {}) {
        const page = Math.max(query.page ?? 1, 1);
        const limit = Math.max(query.limit ?? 10, 1);
        const [rounds, total] = await Promise.all([
    ...
    async updateRoundStatus(roundId, status) {
        ...
        logger_1.logger.info("Interview round status updated", { roundId, status });
        await this._syncParentInterview(updated.applicationId);
        return updated;
    }
    //Reschedule
    async rescheduleRound(roundId, newDate) {
        ...
        logger_1.logger.info("Interview round rescheduled", { roundId, newDate });
        await this._syncParentInterview(updated.applicationId);
        try {
            const details = await this._repository.findByIdWithDetails(roundId);
            if (details) {
                await this._notificationService.notifyCandidateInterviewRescheduled(details.candidateId, details.job.title, new Date(newDate).toISOString(), details.id, details.employer.companyName);
            ...
            logger_1.logger.error("Failed to notify candidate about rescheduled interview", {
                notifyError,
                roundId,
            });
        }
    ...
    //Cancel round
    async cancelRound(roundId, reason) {
        const round = await this._repository.findById(roundId);
        if (!round) {
            throw new Error("Round not found");
        }
        ...
        logger_1.logger.info("Interview round cancelled", { roundId, reason });
        await this._syncParentInterview(updatedWithSync.applicationId);
        try {
            const details = await this._repository.findByIdWithDetails(roundId);
            if (details) {
                await this._notificationService.notifyCandidateInterviewCancelled(details.candidateId, details.job.title, details.id, details.employer.companyName);
            ...
            logger_1.logger.error("Failed to notify candidate about cancelled interview", {
                notifyError,
                roundId,
            });
        }
    ...
    async validateMeetingAccess(roundId, token) {
        ...
        const isValid = (0, linkGenerator_1.validateMeetingToken)(round.meetingToken, token, round.scheduledDate);
        if (!isValid) {
            return { valid: false };
        }
        const roundWithDetails = await this._repository.findByIdWithDetails(roundId);
        return {
            valid: true,
    ...
    async checkAllRoundsComplete(applicationId) {
        const rounds = await this._repository.findByApplicationId(applicationId);
        if (rounds.length === 0) {
            return false;
        }
        return rounds.every((round) => round.status === "completed" || round.status === "cancelled");
    }
    //delete round
    ...
    async _syncParentInterview(applicationId) {
        try {
            const interview = await this._interviewRepository.findByApplicationId(applicationId);
            if (!interview)
                return;
            const rounds = await this._repository.findByApplicationId(applicationId);
            if (rounds.length === 0)
                return;
            const activeRounds = rounds.filter((r) => r.status === "scheduled" || r.status === "rescheduled");
            ...
            logger_1.logger.info("Parent interview synced with rounds", {
                applicationId,
                status: finalStatus,
                date: finalDate,
            });
            ...
            logger_1.logger.error("Failed to sync parent interview", { applicationId, error });
        }
    }
}
exports.InterviewRoundService = InterviewRoundService;
