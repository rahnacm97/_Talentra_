import { IApplication, IApplicationQuery, IApplicationWithJob, IEmployerApplicationResponse } from "./IApplication";
...
export interface ICandidateApplicationRepository {
    ...
    countByCandidateId(candidateId: string, filters?: Partial<Pick<IApplicationQuery, "status" | "search">>): Promise<number>;
    findByCandidateIdWithJob(candidateId: string, query: IApplicationQuery): Promise<IApplicationWithJob[]>;
...
export interface IEmployerApplicationRepository {
    ...
    updateOne(applicationId: string, data: Partial<IApplication>): Promise<IApplication | null>;
...
export interface IApplicationRepository extends ICandidateApplicationRepository, IEmployerApplicationRepository {
    aggregate<T>(pipeline: PipelineStage[]): Promise<T[]>;
    find(query: FilterQuery<IApplication>): Promise<IApplication[]>;
}
//# sourceMappingURL=IApplicationRepository.d.ts.map